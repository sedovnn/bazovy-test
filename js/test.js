...
(function () {
  ...
  function renderDetails(res, rows) {
    ...
    intro.textContent = 'Полоса выше сводит две разные величины в одну шкалу. Тест — ' +
      ...
      'Свободный ответ — что из этого вы сделали сами, уровнем L1–L5.';
  ...
  function demoMap() {
    ...
    var skills = [
      ...
      row('alt', 'Генерация альтернатив', 4, 4, 'ГА-1', 2,
          'Уровень 2 пройден: добавлен новый элемент. Уровень 3 нет: другой стратегической альтернативы не предложено.'),
      row('prio', 'Приоритизация', 3, 4, 'ПР-1', 4,
          'Уровень 4 пройден: отказ сформулирован как правило. Уровень 5 нет: высвобожденный ресурс не назван.'),
      row('future', 'Картина будущего', 4, 4, 'МК-1', 3,
          'Уровень 3 пройден: названо новое положение на рынке с горизонтом. Уровень 4 нет: смены модели не видно.'),
      row('path', 'Путь к цели', 1, 4, 'ПП-1', 2,
          'Уровень 2 пройден: одно действие привязано к цели. Уровень 3 нет: этапы не выстроены от цели.')
    ];
